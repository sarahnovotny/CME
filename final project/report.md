...
**Reproducibility.** All results, tables, and figures are produced by `preprocess.py` followed by `analysis.py` against the inputs documented in `raw_data/README.md`. Random seed = 42 throughout. See `README.md` for run instructions. Full repository: <https://github.com/sarahnovotny/CME/tree/main/final%20project>.
